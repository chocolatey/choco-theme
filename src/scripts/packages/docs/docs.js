import '@choco-theme/lib/prism.js';
import '@choco-theme/get-window-height.js';
import '@choco-theme/alerts.js';
import '@choco-theme/callouts.js';
import '@choco-theme/clipboard.js';
import '@choco-theme/ts/code.ts';
import '@choco-theme/collapse-nested.js';
import '@choco-theme/collapse-responsive.js';
import '@choco-theme/collapse-y-height.js';
import '@choco-theme/docs.js';
import '@choco-theme/packages/docs/src/docsearch.js';
import '@choco-theme/markdown.js';
import '@choco-theme/anchors.js'; // This is dependent on the markdown script above
import '@choco-theme/scrollspy.js';
import '@choco-theme/ts/tab-multiples.ts';
import '@choco-theme/ts/tables.ts';
import '@choco-theme/theme-toggle.js';
import '@choco-theme/dynamic-code-block.js';
